...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        // Scoring
        if (ball.pos.x < 0) { p2Score++; ball = ResetBall(center); }
        else if (ball.pos.x > GetScreenWidth()) { p1Score++; ball = ResetBall(center); }
        ...
        // Movement
        if (IsKeyDown(KEY_W) && p1.y > 0) p1.y -= paddleSpeed * GetFrameTime();
        if (IsKeyDown(KEY_S) && p1.y < GetScreenHeight() - p1.height) p1.y += paddleSpeed * GetFrameTime();
        if (IsKeyDown(KEY_UP) && p2.y > 0) p2.y -= paddleSpeed * GetFrameTime();
        if (IsKeyDown(KEY_DOWN) && p2.y < GetScreenHeight() - p2.height) p2.y += paddleSpeed * GetFrameTime();
